...
fbref %<>% # add column titles, remove non-data rows, refactor
  mutate(data=map(data, fbref_fix_rows))

fbref %<>% # remove cols not required in data
  mutate(data=pmap(list(data, page, type), fbref_tidy))
