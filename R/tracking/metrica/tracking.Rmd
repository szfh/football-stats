...
```{r slice}
fGetTrackingSlice <- function(dtData, subtype, n=1, before=0, after=0){
...
dtTrackingSlice <- fGetTrackingSlice(
  dtData,
  subtype="GOAL KICK", # event type to slice
  n=c(1,2), # which event in the data set. use c() for multiple events.
  before=10, # frames before event
  after=40 # frames after event
)

# dtTrackingSlice <- fGetTrackingSlice(
#   dtData,
#   subtype="REFEREE HIT", # event type to slice
#   n=1, # which event in the data set
#   before=10, # frames before event
#   after=40 # frames after event
# )

# d1 <- dtData %>%
#   filter(Subtype=="REFEREE HIT") %>%
#   tally()
