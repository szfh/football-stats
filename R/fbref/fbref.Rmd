...
players %>%
  ...
  coord_flip()
ggsave(here("plots","SFC","Minutes1.jpg"))
...
players %>%
  ...
  geom_segment(aes(y=Player,yend=Player,x=0,xend=MinStart),colour=col_medium[8],size=2,alpha=0.8) +
  geom_segment(aes(y=Player,yend=Player,x=MinStart,xend=Min),colour=col_medium[1],size=2,alpha=0.8) +
  theme_sfc() +
  labs(title="League minutes (start/sub)",
       x=element_blank(),
       y=element_blank(),
       caption="statsbomb/fbref") +
  expand_limits(Min=0) +
...
ggsave(here("plots","SFC","Minutes2.jpg"))
