...
players %>%
  ...
  geom_point(shape=21,size=3,alpha=0.8,colour="black",fill=sfc) +
...
players %>%
  ...
  geom_point(shape=21,size=2,alpha=0.8,colour="black",fill=sfc) +
  geom_text_repel(aes(label=ifelse(npxG90>0.1|xA90>0.1,Player,"")),size=rel(3)) +
  theme_sfc() +
  labs(title="Southampton xG/xA per 90 minutes",
...
players %>%
  filter(Squad=="Southampton") %>%
  pivot_longer(cols=c(ShortCmp,MediumCmp,LongCmp),names_to="PassType",values_to="Cmp") %>% view
