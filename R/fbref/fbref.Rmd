...
players %>%
  ...
  filter(ShortCmp>0) %>%
  ggplot(aes(x=ShortAtt,y=`ShortCmp%`,label=Player)) +
  geom_point(
    shape=21,
    size=2,
    alpha=0.8,
  ...
  theme_sfc() +
  labs(title="Southampton short passes (MF/FW)",
       x="Short passes attempted",
       y="Short pass completion",
       caption="statsbomb/fbref") +
  scale_x_continuous(breaks=seq(0,500,5),expand=expand_scale(add=c(1.1,1.1))) +
  scale_y_continuous(breaks=seq(0,500,5),labels=function(x) paste0(x,"%"),expand=expand_scale(add=c(17,5)))
ggsave(here("plots","fbref","SFCShortPassComp.jpg"))
```
