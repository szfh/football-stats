...
players %>%
  ...
  scale_y_continuous(breaks=seq(0,90*38,90),expand=expand_scale(add=c(0,50))) +
  coord_flip()
...
squad %>%
  ...
  geom_bar(stat="identity",alpha=0.8) +
  theme_sfc() +
  labs(title="EPL goals - expected goals (non-penalty)",
       x=element_blank(),
       y="xG-G",
       caption="statsbomb/fbref") +
  scale_fill_manual(values=palette_epl()) +
...
squad %>%
  ...
  geom_bar(stat="identity",alpha=0.8) +
  # geom_label(aes(label=`xGA-GA`),size=3) +
  theme_sfc() +
...
squad %>%
  ...
  geom_bar(stat="identity",alpha=0.8) +
  theme_sfc() +
  labs(title="EPL expected goal difference",
       x=element_blank(),
       y="expected goals for - expected goals against",
       caption="statsbomb/fbref") +
  scale_fill_manual(values=palette_epl()) +
  coord_flip()
