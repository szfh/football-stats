...
get_team_players_stats(source[[1]],source[[2]]) %>%
  ...
  coord_fixed()
ggsave(paste0("./plots/",source[[2]],"/",source[[1]],"_xGxA_Point.jpg"))
...
get_team_players_stats(source[[1]],source[[2]]) %>%
  ...
  coord_flip()
ggsave(paste0("./plots/",source[[2]],"/",source[[1]],"_xGxA_Bar.jpg"))
...
get_team_players_stats(source[[1]],source[[2]]) %>%
  ...
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/",source[[2]],"/",source[[1]],"_Shotsper90_bar.jpg"))
...
ggsave(paste0("./plots/",source[[2]],"/",source[[1]],"_Shotsper90_point.jpg"))
...
get_team_players_stats(source[[1]],source[[2]]) %>%
  ...
  filter(xG_per_shot > 0.1) %>%
  filter(shots >= 2) %>%
  ggplot(aes(x=shots,y=xG_per_shot)) +
  geom_point(colour="#e61919",alpha=0.6) +
  geom_text_repel(aes(label=player_name),size=2) +
  labs(title=paste0("League xG per shot ",as.integer(source[[2]]),"-",as.integer(source[[2]])+1),x="Shots",y="xG per shot")
ggsave(paste0("./plots/",source[[2]],"/",source[[1]],"_Shots_xGshot.jpg"))
...
get_team_players_stats(source[[1]],source[[2]]) %>%
  ...
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/",source[[2]],"/",source[[1]],"_xGshot.jpg"))
```
