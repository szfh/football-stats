...
get_team_players_stats(source[[1]],source[[2]]) %>%
  ...
  coord_flip()
ggsave(paste0("./plots/","TeamStats",source[[1]],source[[2]],".jpg"))
...
get_team_players_stats("Southampton",2018) %>%
  ...
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/","PlayerStats",source[[1]],source[[2]],".jpg"))
...
get_team_players_stats("Southampton",2018) %>%
  ...
  geom_text(aes(label=ifelse(xG+xA>=2,player_name,""),hjust="right",vjust="outward"),size=2.5) +
  labs(title=paste0("League xG/xA ",as.integer(source[[2]]),"-",as.integer(source[[2]])+1),x="xG",y="xA") +
  scale_shape_identity()
ggsave(paste0("./plots/","xGxA",source[[1]],source[[2]],".jpg"))
