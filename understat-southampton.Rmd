...
```{r plots}
get_team_players_stats(team,year) %>%
  filter(time>0) %>%
  mutate(player_name=factor(player_name,levels=player_name[order(time)])) %>%
  ggplot(aes(x=player_name,y=time)) +
  geom_bar(stat="identity",fill=colour_sfc1,width=0.8,alpha=0.6) +
  labs(title=paste0("League minutes ",as.integer(year),"-",as.integer(year)+1)) +
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/",year,"/",team,"_minutes.jpg"))
...
get_team_players_stats(team,year) %>%
  ...
  geom_bar(stat="identity",fill=colour_sfc1,width=0.8,alpha=0.6) +
  labs(title=paste0("League xG ",as.integer(year),"-",as.integer(year)+1)) +
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/",year,"/",team,"_xG",".jpg"))

get_team_players_stats(team,year) %>%
  filter((xG > 0)|(xA > 0)) %>%
  ggplot(aes(x=xG,y=xA)) +
  geom_point(colour=colour_sfc1,alpha=0.6) +
  ...
  labs(title=paste0("League xG/xA ",as.integer(year),"-",as.integer(year)+1),x="xG",y="xA") +
  coord_fixed()
ggsave(paste0("./plots/",year,"/",team,"_xG_xA_point.jpg"))
...
get_team_players_stats(team,year) %>%
  ...
  labs(title=paste0("League xG/xA ",as.integer(year),"-",as.integer(year)+1)) +
  theme(axis.title.x=element_blank(),axis.title.y=element_blank(),legend.title=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/",year,"/",team,"_xG_xA_bar.jpg"))
...
get_team_players_stats(team,year) %>%
  ...
  geom_bar(stat="identity",fill=colour_sfc1,width=0.8,alpha=0.6) +
  labs(title=paste0("Shots ",as.integer(year),"-",as.integer(year)+1)) +
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/",year,"/",team,"_shots.jpg"))
...
get_team_players_stats(team,year) %>%
  ...
  geom_bar(stat="identity",fill=colour_sfc1,width=0.8,alpha=0.6) +
  labs(title=paste0("Shots per 90 ",as.integer(year),"-",as.integer(year)+1)) +
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/",year,"/",team,"_shots_per_90_bar.jpg"))
...
get_team_players_stats(team,year) %>%
  ...
  labs(title=paste0("Shots per 90 ",as.integer(year),"-",as.integer(year)+1),x="Minutes",y="Shots per 90")
ggsave(paste0("./plots/",year,"/",team,"_shots_per_90_point.jpg"))

get_team_players_stats(team,year) %>%
  filter(key_passes > 0) %>%
  mutate(key_passes_per_90=(shots/time)*90) %>%
  filter(key_passes_per_90 > 0.2) %>%
  ggplot(aes(x=time,y=key_passes_per_90)) +
  geom_point(colour=colour_sfc1,alpha=0.6) +
  geom_text_repel(aes(label=player_name),size=2) +
  labs(title=paste0("Key passes per 90 ",as.integer(year),"-",as.integer(year)+1),x="Minutes",y="Key passes per 90")
ggsave(paste0("./plots/",year,"/",team,"_keypasses_per_90_point.jpg"))
...
get_team_players_stats(team,year) %>%
  ...
  geom_bar(stat="identity",fill=colour_sfc1,width=0.8,alpha=0.6) +
  labs(title=paste0("Key passes ",as.integer(year),"-",as.integer(year)+1)) +
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/",year,"/",team,"_keypasses.jpg"))
...
get_team_players_stats(team,year) %>%
  ...
  geom_point(colour=colour_sfc1,alpha=0.6) +
  geom_text_repel(aes(label=player_name),size=2) +
  labs(title=paste0("xG per shot ",as.integer(year),"-",as.integer(year)+1),x="Shots",y="xG per shot")
ggsave(paste0("./plots/",year,"/",team,"_shots_xG_per_shot.jpg"))
...
get_team_players_stats(team,year) %>%
  ...
  geom_bar(stat="identity",fill=colour_sfc1,width=0.8,alpha=0.6) +
  labs(title=paste0("League xG per shot ",as.integer(year),"-",as.integer(year)+1)) +
  theme(axis.title.x=element_blank(),axis.title.y=element_blank()) +
  coord_flip()
ggsave(paste0("./plots/",year,"/",team,"_xG_per_shot.jpg"))
```

### Test plots

```{r test plots,echo=F,eval=T}
get_team_players_stats("Southampton",2019) %>%
  filter(player_name == "Che Adams") %>%
  select(player_id,player_name)

get_league_teams_stats("EPL",2019) %>%
